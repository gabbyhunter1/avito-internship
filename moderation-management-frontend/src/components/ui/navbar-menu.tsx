...
export const MenuItem = ({
  ...
  return (
    <div onMouseEnter={() => setActive(item)} className="relative ">
      <motion.p transition={{ duration: 0.3 }} className="cursor-pointer text-white hover:opacity-[0.9] dark:text-black">
        {item}
      </motion.p>
      {active !== null && (
...
export const Menu = ({ setActive, children }: { setActive: (item: string | null) => void; children: React.ReactNode }) => {
  return (
    <nav
      ...
      className="relative rounded-full border border-transparent dark:bg-white dark:border-white/[0.2] bg-black shadow-input flex justify-center space-x-4 px-8 py-6 ">
      {children}
    </nav>
  );
};
