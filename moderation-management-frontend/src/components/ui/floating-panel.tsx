...
export function FloatingPanelTrigger({ children, className, title }: FloatingPanelTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        'flex h-9 items-center border border-neutral-200 border-zinc-950/10 bg-white px-3 text-zinc-950 dark:border-zinc-50/10 dark:bg-zinc-700 dark:text-zinc-50 dark:border-neutral-800',
        className
      )}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              'fixed z-50 overflow-hidden border border-neutral-200 border-zinc-950/10 bg-white shadow-lg outline-none dark:border-zinc-50/10 dark:bg-zinc-800 dark:border-neutral-800',
              className
            )}
            style={{
              borderRadius: 12,
...
export function FloatingPanelBody({ children, className }: FloatingPanelBodyProps) {
  return (
    <motion.div className={cn('p-4', className)} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
      {children}
    </motion.div>
  );
}
...
export function FloatingPanelSubmitButton({ className }: FloatingPanelSubmitButtonProps) {
  return (
    <motion.button
      className={cn(
        'relative ml-1 flex h-8 shrink-0 scale-100 select-none appearance-none items-center justify-center rounded-lg border border-neutral-200 border-zinc-950/10 bg-transparent px-2 text-sm text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-800 focus-visible:ring-2 active:scale-[0.98] dark:border-zinc-50/10 dark:text-zinc-50 dark:hover:bg-zinc-800 dark:border-neutral-800',
        className
      )}
      type="submit"
...
export function FloatingPanelButton({ children, onClick, className }: FloatingPanelButtonProps) {
  return (
    <motion.button
      className={cn('flex w-full items-center gap-2 rounded-md px-4 py-2 text-left text-sm hover:bg-zinc-100 dark:hover:bg-zinc-700', className)}
      onClick={onClick}
      whileHover={{ backgroundColor: 'rgba(0, 0, 0.05)' }}
      whileTap={{ scale: 0.98 }}>
      {children}
    </motion.button>
  );
